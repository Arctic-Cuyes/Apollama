...
class PostService {
  ...
  bool _thisPostHasAnyTag(Post post, List<Tag> tags) {
    List<Tag>? postTags = post.tagsData;
    return tags.any(
        (tag) => postTags!.any((postTag) => postTag.name == tag.name) == true);
  }
  ...
  Future<bool> _thisPostMustBeInactive(Post post) async {
    if (post.active &&
        DateTime.now().isAfter(post.endDate.add(const Duration(days: 1)))) {
